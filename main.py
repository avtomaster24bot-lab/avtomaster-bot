...
from aiogram.fsm.storage.memory import MemoryStorage  # SQLite FSM
...
def validate_env():
    ...
    if not os.path.exists("avtomaster.db"):
        logging.warning("База данных avtomaster.db ещё не создана")
...
async def run_migrations():
    ...
    try:
        ...
        logging.warning(f"Не удалось применить миграции: {e}. init_db создаст базу.")
...
def register_routers(dp: Dispatcher):
    ...
    for r in admin_routers + client_routers:
        dp.include_router(r)

    logging.info(f"Роутеры активированы: {len(admin_routers)} админ, {len(client_routers)} клиент")
...
async def main():
    ...
    # FSM через MemoryStorage (SQLite)
